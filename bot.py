...
weather_api_token = "7bf3cd3817mshf50b92f26dab764p1f5ddajsn2fa97d5600ec"
wit_ai_token = "LJBUFFWNHFCKOGZ2BEGPZKZV4IZJTV5S"
...
class Bot(object):
    ...
    def decide_action(self):
        ...
        # received audio data, now  recognize it using Google Speech Recognition
        ...
        if speech is not None:
            try:
                ...
                querystring = {}
                querystring['q'] = speech
                r = requests.request("GET", "https://api.wit.ai/message", headers=headers, params=querystring)
               # print (r.text)
                json_resp = json.loads(r.text)
                entities = None
                intent = None
                if 'entities' in json_resp and 'intents' in json_resp:
                    entities = json_resp['entities']
                    print(json_resp)
                    if (len(json_resp['intents']) is not 0):
                        intent = json_resp['intents'][0]['name']
                print (intent)
                if intent == 'greeting':
                    self.__text_action(self.nlg.greet())
                elif intent == 'snow_white':
                    ...
                    #return
                elif intent == 'meaning_of_life':
                    self.__meaning_of_life_action()
    ...
    def __user_status_action(self, nlu_entities=None):
        ...
        print(nlu_entities)
        if (nlu_entities is not None) and ('Status-Type:Status-Type' in nlu_entities):
            print("GOES IN FOIJKSIEKWJDHSNFOEIJFKDN")
            attribute = nlu_entities['Status-Type:Status-Type'][0]['value']
    ...
    def __meaning_of_life_action(self):
    ...
    def __maps_action(self, nlu_entities=None):
        ...
        #print(nlu_entities['wit$location:location'][0])
        if nlu_entities is not None:
            if 'wit$location:location' in nlu_entities:
                #location = nlu_entities['location'][0]["value"]
                location = nlu_entities['wit$location:location'][0]['resolved']['values'][0]['name']
    ...
    def __holidays_action(self):
        ...
        self.speech.synthesize_text(self.nlg.holiday(next_holiday['name'][1]["text"]))
